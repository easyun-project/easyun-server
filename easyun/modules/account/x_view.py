...
class SSHKeys(MethodView):
    ...
    @bp.input(CreateSSHKeySchema)
    @bp.output(SSHKeysOutputSchema)
    def post(self, data):
        try:
...
class FreeTier(MethodView):
    ...
    @bp.output(FreeTierOutputSchema)
    def get(self):
...
# @bp.post('/dev')
# @bp.input({"key": String(), "value": String(validate=Length(0, 2048))})
# def dev(data):
