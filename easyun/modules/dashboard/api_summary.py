...
def summary_dc(parm):
    ...
    regionDict = next(
        (region for region in AWS_Regions if region['regionCode'] == dcRegion), None
    )
    regionInfo = {
    ...
    for az in azs['AvailabilityZones']:
        ...
        subnetResp = client_ec2.describe_subnets(
            Filters=[
                ...
                {'Name': 'availability-zone', 'Values': [azName]},
            ],
        )
...
def summary_resource(parm):
    ...
    stblockSum = {
        ...
        "data": {
            ...
            "blcSize": {'value': sum([int(i) for i in sizeList]), 'unit': 'GiB'},
    ...
    stfileSum = {
        ...
        "data": {
            ...
            "efsSize": {'value': 827.7, 'unit': 'MiB'},
            "fsxNum": 2,
            "fsxSize": {'value': 3.27, 'unit': 'GiB'},
        },
    }

    resp = Result(
        detail=[serverSum, databaseSum, networkSum, stobjectSum, stblockSum, stfileSum]
    )
    return resp.make_resp()
